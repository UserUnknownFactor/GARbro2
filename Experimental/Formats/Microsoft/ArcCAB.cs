...
namespace GameRes.Formats.MSFormats
{
    internal sealed class CabEntry : Entry
    {
        ...
        public CabEntry(CFFile fileInfo, CFFolder folder)
        {
    ...
    public sealed class CabOpener : ArchiveFormat
    {
        ...
        public byte[] cab_header = { (byte)'M', (byte)'S', (byte)'C', (byte)'F' };
        ...
        public override ArcFile TryOpen(ArcView file)
        {
            ...
            try
            {
                ...
                foreach (var folder in cabHeader.cfFolders)
                {
                    foreach (var fileInfo in folder.GetFiles())
                        dir.Add(new CabEntry(fileInfo, folder));
                }

                if (!dir.Any())
                    return null;

                return new CabArcFile(file, this, dir, cabHeader, base_offset);
            }
            catch
            {
                return null;
    ...
    internal class CabArcFile : ArcFile
    {
        ...
        public CabArcFile(ArcView arc, ArchiveFormat impl, ICollection<Entry> dir, CFHeader header, long baseOffset)
            : base(arc, impl, dir)
        {
            Header = header;
    ...
    internal class CFFolder
    {
        ...
        private object decompressionLock = new object();
