...
namespace GameRes.Formats.RbPack
{
    ...
    public class RbPackOpener : ArchiveFormat
    {
        ...
        public override ArcFile TryOpen (ArcView file)
        {
            ...
            //var mainZipEntries = new HashSet<string>(); // to avoid collisions with resources
            ...
            if (rbEntry.IsMainZip)
            {
                ...
                using (var zipStream = new MemoryStream())
                {
                    ...
                    using (var zip = new ICSharpCode.SharpZipLib.Zip.ZipFile (zipStream))
                    {
                        ...
                        var zipEntry = zip.GetEntry (rbEntry.ZipName);
                        if (zipEntry == null)
                            throw new FileNotFoundException (rbEntry.ZipName);
