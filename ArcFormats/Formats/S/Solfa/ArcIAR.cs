...
namespace GameRes.Formats.Solfa
{
    ...
    public class IarOpener : ArchiveFormat
    {
        ...
        public override ArcFile TryOpen (ArcView file)
        {
            ...
            if (dir.Count == 0)
                throw new NotSupportedException("Unsupported IAR format");
    ...
    internal class IarImage
    {
        ...
        public IarImage (IarArchive iarc, Entry entry)
        {
            ...
            default:    throw new NotSupportedException ("Unsupported IAR image format");
            }
    ...
    internal sealed class IarDecompressor : IDisposable
    {
        ...
        public void Unpack (byte[] output)
        {
            ...
            while (dst < output.Length)
            {
                ...
                {
                    int tmp = GetNextBit();
                    if (1 == GetNextBit())      offset = 1;
                    else if (1 == GetNextBit()) offset = 0x201;
                    else
                    {
                        tmp = (tmp << 1) | GetNextBit();
                        if (1 == GetNextBit()) offset = 0x401;
                        else
                        {
                            tmp = (tmp << 1) | GetNextBit();
                            if (1 == GetNextBit()) offset = 0x801;
                            else
                            {
                                offset = 0x1001;
                                tmp = (tmp << 1) | GetNextBit();
                    ...
                    if (1 == GetNextBit())      count = 3;
                    else if (1 == GetNextBit()) count = 4;
                    else if (1 == GetNextBit()) count = 5;
                    else if (1 == GetNextBit()) count = 6;
                    else if (1 == GetNextBit()) count = 7 + GetNextBit();
                    else if (1 == GetNextBit()) count = 17 + m_input.ReadUInt8();
                    else
                    {
                        count  = GetNextBit() << 2;
                        count |= GetNextBit() << 1;
