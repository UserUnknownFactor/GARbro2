...
namespace GameRes.Formats.Cyberworks
{
    ...
    public class DataOpener : ArchiveFormat
    {
        ...
        {
            if (0 == entry.Size) 
                throw new FileSizeException(Localization._T ("MsgFileIsEmpty"));     //in some cases,the size of entry maybe 0,like game Mujina(vndb.org/v48764).
        ...
        DataScheme QueryScheme(string arc_name) 
        {
            ...
            if (!string.IsNullOrEmpty(title) && KnownSchemes.TryGetValue(title, out scheme))
                return scheme;
            var options = Query<DataOptions>(Localization._T ("ArcEncryptedNotice"));
            if (null != options)
