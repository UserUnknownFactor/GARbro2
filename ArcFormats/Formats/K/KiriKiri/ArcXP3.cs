...
namespace GameRes.Formats.KiriKiri
{
    ...
    public class Xp3Opener : ArchiveFormat
    {
        ...
        public override string Description { get { return Localization._T ("XP3Description"); } }
        ...
        {
            long base_offset = ExeFile.FindSignature (file, s_xp3_header); // 0 if it's not exe
...
NextEntry:
        ...
        ICrypt QueryCryptAlgorithm (ArcView file)
        {
            ...
            if (null != alg)
                return alg;
            var options = Query<Xp3Options> (Localization._T ("XP3EncryptedNotice"));
                                     ...
                                     EntryCallback callback)
        {
            ...
            using (var writer = new BinaryWriter (output, Encoding.ASCII, true))
            {
                ...
                using (var header = new BinaryWriter (new MemoryStream (dir.Count*0x58), Encoding.Unicode))
                {
                    ...
                    {
                        if (null != callback)
                            callback (callback_count++, null, Localization._T ("MsgCompressingIndex"));
