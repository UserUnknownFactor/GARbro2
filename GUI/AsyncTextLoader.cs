...
namespace GARbro.GUI.Preview
{
    public class AsyncTextLoader
    {
            ...
            Encoding preferredEncoding, CancellationToken cancellationToken)
        {
            ...
            if (scriptData == null)
            {
                return new TextLoadResult { Error = "Failed to read script data" };
            }
            ...
            CancellationToken cancellationToken)
        {
            ...
            // Text file - detect encoding if needed
            Encoding encodingToUse = preferredEncoding;
            if (encodingToUse == null)
            {
                stream.Position = 0;
                encodingToUse = ScriptFormat.DetectEncoding (stream, Math.Min (stream.Length, 20000));
            }
